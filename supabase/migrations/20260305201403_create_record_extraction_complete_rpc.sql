/*
  # Create record_extraction_complete RPC
  
  1. Purpose
    - Called by policy extraction worker to commit extracted clauses
    - Updates policy_document status
    - Creates policy_version if extraction is complete
    - Queues PENDING_REVIEW clauses to clause_review_queue
  
  2. Security
    - SECURITY DEFINER to allow worker to insert clauses
    - Validates all required fields including source_citation
*/
...
GRANT EXECUTE ON FUNCTION record_extraction_complete(uuid,jsonb,text,text,text,uuid) TO service_role;