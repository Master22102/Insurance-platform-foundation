/*
  # Fix projector: guard orphaned incident FK references

  ## Summary
  event_ledger rows with scope_type='incident' may reference incidents that were
  deleted (test cleanup, ON DELETE CASCADE, etc.).
  The projector must check that the incident still exists before setting incident_id
  on the action_inbox_items row — otherwise the FK constraint fires.

  ## Fix
  Add an existence check: if scope_type='incident' but the incident no longer
  exists in the incidents table, set incident_id = NULL instead of failing.
*/
...
BEGIN
  ...
  LOOP
    ...
    -- Resolve incident_id — only if incident still exists
    v_incident_id := NULL;
    IF v_event.scope_type = 'incident' AND v_event.scope_id IS NOT NULL THEN
      IF EXISTS (SELECT 1 FROM incidents WHERE id = v_event.scope_id) THEN
        v_incident_id := v_event.scope_id;
      END IF;
    END IF;
